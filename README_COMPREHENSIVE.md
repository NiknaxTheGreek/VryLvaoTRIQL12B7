...
The model is therefore designed to produce a **ranking signal**. It is not intended to:

- determine product eligibility;
- approve or reject customers;
- set prices or credit limits;
- guarantee who will subscribe;
- replace consent or contact-frequency rules;
- provide causal estimates of how much calling itself changes subscription behaviour.
...
Logistic Regression is the interpretable linear baseline and final pilot candidate. It is attractive operationally because:

- coefficients and preprocessing are transparent;
- the model is straightforward to monitor;
- score changes are easier to investigate;
- it provides a strong reference against which more complex models can be justified.
...
## 5. Reproducible Validation and Tuning Design
...
This hyperparameter search is based on one fixed train/validation split. It is **not five-fold cross-validation tuning**.

### Logistic Regression search

The LR search evaluates:

- 15 log-spaced `C` values from 0.001 to 100;
- L1 and L2 regularisation;
- unweighted and balanced class weighting.

That gives **60 fixed configurations per feature condition**.

### HistGradientBoosting search

The HGB search space contains:
...
The full Cartesian space contains **28,800 combinations**. Rather than exhaustively fitting all of them, `ParameterSampler(..., n_iter=100, random_state=42)` selects the **same 100 candidates every rerun for each feature condition**.

Across LR pre-call, LR with duration, HGB pre-call and HGB with duration, the notebook evaluates exactly **320 candidate configurations**.

The primary tuning objective is **PR-AUC**, with **ROC-AUC as tie-breaker**.

## 6. Selected Hyperparameters

| Configuration | Selected parameters |
|---|---|
| LR pre-call | L2, `C=8.483428982440726`, class weight None |
| LR + duration | L1, `C=0.0610540229658533`, class weight None |
| HGB pre-call | `max_iter=200`, `max_depth=8`, `learning_rate=0.05`, `max_leaf_nodes=15`, `l2_regularization=0`, `min_samples_leaf=20`, class weight None, early stopping |
| HGB + duration | `max_iter=200`, `max_depth=3`, `learning_rate=0.05`, `max_leaf_nodes=127`, `l2_regularization=2`, `min_samples_leaf=10`, class weight balanced, early stopping |
...
## 7. Default, Tuned and Leakage Results
...
| Default | LR | + duration | 0.938125 | 0.655556 | 0.305699 | 0.416961 | 0.525049 | 0.932379 |
| Default | HGB | Pre-call | 0.928000 | 0.534884 | 0.039724 | 0.073955 | 0.268264 | 0.734102 |
| Default | HGB | + duration | 0.941125 | 0.639896 | 0.426598 | 0.511917 | 0.585190 | 0.951528 |
...
Negative permutation values mean the held-out score did not improve when that feature was preserved in that particular perturbation estimate; they should not be interpreted as proof that the variable is intrinsically harmful in every setting.
...
HGB Top-4 has slightly higher PR-AUC than HGB All-12 on this one exploratory holdout comparison, but All-12 has materially stronger ROC-AUC. No subset-specific retuning is performed. The evidence therefore does not justify replacing the full valid pre-call set in the final pilot specification.

## 9. Class-Imbalance Experiments

The project compares six strategies on the tuned pre-call model structures:
...
Thresholds are selected using five-fold stratified out-of-fold predictions inside development data and then applied once to the untouched holdout:
...
The ranking recommendation therefore remains **unweighted**. Weighting and resampling alter operating trade-offs, but they do not provide a material general ranking advantage.
...
For predictive validation, the Q3 threshold is learned within each training fold rather than from the full dataset, and the full-population **default** model structures are used without subgroup-specific hyperparameter tuning.
...
The dataset has no verified customer identifier. To test sensitivity to potentially similar records appearing across folds, exact combinations of:

`age + job + marital + education + balance`

are used as pseudo-groups in `StratifiedGroupKFold`.
...
The ordered robustness design trains on all eligible earlier blocks and tests on a later contiguous month-labelled block. A test block is included only when there are sufficient earlier training rows and enough positive and negative test outcomes for meaningful metric calculation.

Eight blocks pass the eligibility gate, covering:
...
Calibration is assessed period by period using local quantile bins, Brier score and expected calibration error (ECE).

No sigmoid calibration model is fitted. No isotonic calibration model is fitted.
...
A lift of 1.0 means no enrichment over random selection. A lift of 1.85 means the selected group contains about 1.85 times as many subscribers per call as random calling at that depth, historically within the evaluated blocks.
...
These are retrospective sensitivity points. They are **not prescribed production quotas**. Actual call depth should be determined by staff capacity, contact cost, expected contribution from a successful subscription and risk controls.
...
LR is not chosen because it wins every historical metric. It does not. HGB has stronger shuffled PR-AUC and ROC-AUC. LR is chosen because the model-selection decision incorporates:

- later-block ordered robustness;
- period-local historical lift;
- transparency;
- monitoring simplicity;
- limited benefit from additional imbalance complexity;
- the need for a conservative controlled pilot rather than immediate production rollout.

## 16. Controlled Pilot Design

The next business step is a prospective test, not further retrospective score polishing.

A suitable pilot compares **equal calling capacity** across:

- a model-ranked LR arm;
- a business-as-usual or random comparator arm;
- optionally, an HGB challenger arm if operational capacity allows.

The primary business outcome should be **subscriptions per 1,000 assigned calls**.
...
Once those values are supplied, the operational comparison can be written as:
...
| Default LR/HGB with and without duration | `02_baseline_tuning.ipynb` | Leakage and baseline comparison |
| LR grid and HGB seeded random search | `02_baseline_tuning.ipynb` | Reproducible hyperparameter selection |
| Exact selected parameters | `02_baseline_tuning.ipynb` | Model specification |
| Untouched 20% holdout table and six-metric panel | `02_baseline_tuning.ipynb` | Core historical evaluation |
| HGB permutation ranking | `03_features_imbalance.ipynb` | Feature-reduction order |
...
| Exploratory population predictive validation | `04_segments_robustness.ipynb` | Segment robustness |
